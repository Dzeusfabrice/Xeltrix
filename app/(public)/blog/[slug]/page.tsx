...
export default async function ArticleDetailPage({ params }: { params: { slug: string } }) {
    ...
    return (
        <div className="min-h-screen bg-[#020617] pb-32">
            ...
            <header className="relative py-48 overflow-hidden border-b border-white/5">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
                    ...
                    <aside className="lg:col-span-1 hidden lg:block sticky top-32 h-fit">
                        <div className="flex flex-col gap-6 items-center">
                            <span className="text-[10px] uppercase font-black tracking-widest text-slate-700 [writing-mode:vertical-lr]">Partager</span>
                            <div className="w-px h-12 bg-white/5" />
                            <Link href="#" className="p-3 bg-white/5 hover:bg-white/10 rounded-2xl text-slate-400 hover:text-white transition-all border border-white/5">
                                <Linkedin size={18} />
                            </Link>
                            <Link href="#" className="p-3 bg-white/5 hover:bg-white/10 rounded-2xl text-slate-400 hover:text-white transition-all border border-white/5">
                                <Twitter size={18} />
                    ...
                    <article className="lg:col-span-8 space-y-12">
                        {article.excerpt && (
                            <p className="text-2xl md:text-3xl text-slate-300 font-bold leading-relaxed italic border-l-4 border-purple-600 pl-8 py-4 bg-purple-600/5 rounded-r-3xl">
                                {article.excerpt}
                            </p>
                        )}

                        <div
                            className="prose prose-invert prose-purple max-w-none text-xl text-slate-400 font-medium leading-[1.8] space-y-8"
                        ...
                        <div className="pt-24 border-t border-white/5">
                            <div className="p-12 rounded-[3.5rem] bg-slate-900 shadow-2xl border border-white/5 relative overflow-hidden group glass-card">
                                ...
                                <div className="space-y-8 relative z-10 text-center md:text-left">
                                    <div className="flex flex-col md:flex-row items-center gap-8">
                                        <div className="w-24 h-24 rounded-4xl bg-gradient-to-br from-purple-600 to-blue-600 p-1 shrink-0">
                                            <div className="w-full h-full rounded-[inherit] bg-slate-900 flex items-center justify-center font-black text-4xl text-white">X</div>
                                        </div>
                                        <div className="space-y-2">
                                            <h4 className="text-2xl font-black text-white">À propos de Xeltrix</h4>
                                    ...
                                    <div className="pt-4">
                                        <Link href="/contact">
                                            <Button className="h-14 px-8 rounded-2xl bg-white text-slate-950 hover:bg-slate-100 uppercase tracking-widest text-xs font-black">
                    ...
                    <aside className="lg:col-span-3 space-y-12">
                        <section className="space-y-6">
                            ...
                            <div className="p-8 rounded-[2rem] bg-white/5 border border-white/5 space-y-6">
                                <p className="text-sm text-slate-400 font-medium">Rejoignez 500+ décideurs tech pour nos analyses exclusives.</p>
                                <div className="space-y-3">
                                    <input
                                        type="email"
                                        placeholder="Votre email..."
                                        className="w-full bg-slate-950 border border-white/10 rounded-xl py-4 px-4 text-xs font-bold text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50"
                                    />
                        ...
                        <section className="space-y-6 sticky top-32">
                            ...
                            <div className="space-y-3">
                                {['Technologie', 'Design', 'Cloud', 'Business'].map(tag => (
                                    <Link key={tag} href={`/blog?filter=${tag.toLowerCase()}`} className="block p-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 text-slate-400 hover:text-white transition-all text-xs font-black uppercase tracking-widest group">
