...
const WallIcon = ({ icon, delay = 0 }: { icon: React.ReactNode, delay?: number }) => (
    <motion.div
        ...
        className="tech-icon-item reflection w-16 h-16 md:w-20 md:h-20 bg-slate-900/50 border border-white/10 rounded-2xl flex items-center justify-center text-3xl shadow-2xl backdrop-blur-sm"
    >
        {icon}
    </motion.div>
)
...
export default function TechnologiesClient({ initialTech }: { initialTech: any[] }) {
    ...
    return (
        <div className="min-h-screen bg-[#020617] pt-12 pb-24 overflow-hidden">
            {/* Header Section */}
            ...
            <Container className="relative z-10">
                ...
                <div className="mt-20 mb-32 relative py-20 tech-icon-wall text-center">
                    <div className="flex flex-wrap justify-center gap-6 md:gap-10 max-w-5xl mx-auto">
                        <WallIcon icon={<Cpu className="text-blue-400" size={32} />} delay={0.1} />
                        <WallIcon icon={<Globe className="text-purple-400" size={32} />} delay={0.2} />
                        <WallIcon icon={<Smartphone className="text-cyan-400" size={32} />} delay={0.3} />
                        <WallIcon icon={<Database className="text-emerald-400" size={32} />} delay={0.4} />
                        <WallIcon icon={<Cloud className="text-orange-400" size={32} />} delay={0.5} />
                        <WallIcon icon={<Terminal className="text-pink-400" size={32} />} delay={0.6} />
                        <WallIcon icon={<Server className="text-indigo-400" size={32} />} delay={0.7} />
                        <WallIcon icon={<Network className="text-teal-400" size={32} />} delay={0.8} />
                    </div>
                </div>

                {/* Filters & Search Bar */}
                <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-16 bg-slate-900/40 p-4 rounded-3xl border border-white/5 backdrop-blur-xl">
                    ...
                    <div className="relative w-full md:w-96">
                        ...
                        <input
                            ...
                            className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all font-bold placeholder:text-slate-700"
                        />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 text-left">
                    {filteredTech.map((tech, index) => (
                        <motion.div
                            ...
                            className="group p-8 rounded-[2rem] bg-slate-900/30 border border-white/5 hover:border-blue-500/30 transition-all glass-card flex flex-col h-full"
                        >
                            <div className="flex items-start gap-4 mb-8">
                                <div className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-2xl group-hover:scale-110 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500 p-3">
                                    {tech.logo_url ? (
                                ...
                                <div className="space-y-1 pt-1">
                                    ...
                                    <p className="text-[10px] uppercase font-black text-blue-400 tracking-[0.2em]">{tech.category}</p>
                                </div>
                            </div>
                            ...
                            <div className="pt-6 border-t border-white/5">
                                <div className="flex items-center justify-between mb-2">
                                    <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Maîtrise</span>
                                    <span className="text-xs font-black text-white">{tech.proficiency}%</span>
                                </div>
                ...
                {filteredTech.length === 0 && (
                    <div className="py-20 text-center">
                        <p className="text-slate-500 text-xl font-black italic tracking-tight">Aucune technologie ne correspond à votre recherche.</p>
                    </div>
                )}
            </Container>
        </div>
